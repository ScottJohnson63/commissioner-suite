"""
Weekly sync — runs every Tuesday 08:00 UTC.
Only executes between August 1 and February 1 (NFL season window).
Syncs only the most recently completed week.
"""
...
def is_in_season() -> bool:
    """Only run between August 1 and February 1."""
    if FORCE:
...
def fetch_current_week(season: int) -> pl.DataFrame:
    ...
    print(f"  {len(df)} rows for week {max_week}")
    return df
...
def upsert_rows(df: pl.DataFrame) -> None:
    ...
    for i in range(0, len(rows), chunk_size):
        ...
        total += len(chunk)
        if total % 200 == 0 or total == len(rows):
...
def main() -> None:
    ...
    df = fetch_current_week(CURRENT_SEASON)
